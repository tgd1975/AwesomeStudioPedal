...
Env:
  ASP_SKIP_SECURITY_REVIEW=1  bypass entirely (escape hatch)
  ASP_SKIP_CLAUDE_REVIEW=1    skip the semantic Claude pass, static rules only
"""
...
def run_claude_review(diff: str) -> tuple[str | None, str | None]:
    ...
    if os.environ.get("ASP_SKIP_CLAUDE_REVIEW") == "1":
        return None, "ASP_SKIP_CLAUDE_REVIEW=1"
    claude = find_claude_cli()
    if not claude:
        return None, "claude CLI not found (set ASP_CLAUDE_BIN or install on PATH)"
    if not diff.strip():
...
def main() -> int:
    ...
    if os.environ.get("ASP_SKIP_SECURITY_REVIEW") == "1":
        print("[security-review] skipped (ASP_SKIP_SECURITY_REVIEW=1)", file=sys.stderr)
        return 0
