...
def wait_for_ready(port: str) -> serial.Serial:
    """Open serial port and block until firmware prints '[BLE_TEST] READY'."""
...
def read_serial_line(ser: serial.Serial, timeout: float = 5.0) -> str | None:
    """Read lines until one matches [BLE_TEST] or timeout."""
...
async def find_pedal(timeout: float = BLE_SCAN_TIMEOUT) -> object:
    """Scan for the pedal advertising the Config service UUID."""
    print(f"Scanning for pedal ({timeout} s)…")
    device = await BleakScanner.find_device_by_filter(
        lambda d, adv: pedal_config.SERVICE_UUID.lower() in [
...
async def run_tests(ser: serial.Serial) -> int:
    ...
    # Read serial for profile name
    serial_line = read_serial_line(ser, timeout=5.0) or ""
    ...
    if device2 is None:
        ...
        ser.write(b"PROFILE?\n")
        post_line = read_serial_line(ser, timeout=5.0) or ""
