...
- 2026-04-26: moved back from `active/` to `open/` — Tobias does not currently
  have an iPhone, and the work is fundamentally blocked on hardware acquisition
  rather than on a sibling task we can finish. Pick this up once an iPhone is
  available.

## Paused

- 2026-04-27: Paused — no iPhone is currently on hand. Every iOS-* test in
  this task requires a physical device (the simulator does not support BLE),
  so execution is fully blocked on hardware acquisition. Resume with
  `/ts-task-active TASK-158` once an iPhone is available.
